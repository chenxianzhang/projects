<template>
  <div>
    ...
    <el-table :data="tableData" style="width: 100%" border :height="tableHeight" :header-cell-style="{background:'rgba(28, 77, 125, 0.8)', color:'white', fontWeight:'bold'}">
      <el-table-column v-for="(task,index) in columns" :label="task.label" :prop="task.prop" align="center" :key="index">
        <template slot-scope="scope">
          <div v-if="task.type && task.type==='link'" @click.stop="viewScoreDetail(task.taskId, scope.row.studentNo, scope.row[task.prop])" style="cursor: pointer; color: #ee9900;">
            {{scope.row[task.prop]}}
          </div>
...
export default {
  ...
  created() {
    ...
    getCourseById({ courseId: cId }).then(resp => {
      if (resp.status === 0) {
  ...
  methods: {
    ...
    transferData(data) {
      ...
      for (let item = 0; item < data.length; item++) {
        ...
        data[item].studentTaskInfos &&
          data[item].studentTaskInfos.length !== 0 &&
    ...
    statisticTaskScore() {
      ...
      statStudentTaskScore({
        ...
        this.taskStaticChart = this.$echarts.init(
          this.$refs.statisticTaskScoreEl
        )
          ...
          series: [
            {
              ...
              min: 0,
              max:
                resp.data.totalStudentCnt / 5 === 0
                  ? resp.data.totalStudentCnt
                  : (resp.data.totalStudentCnt / 5 + 1) * 5,
              splitNumber:
                resp.data.totalStudentCnt / 5 === 0
                  ? resp.data.totalStudentCnt
                  : resp.data.totalStudentCnt / 5 + 1,
              radius: '85%',
              axisLine: {
                // 坐标轴线
                lineStyle: {
            ...
            },
            {
              ...
              data: [
                {
                  value:
                    !resp.data.totalScore || resp.data.totalScore === ''
                      ? 0
            ...
            },
            {
              ...
              data: [
                {
                  value: resp.data.objectiveItemScoreRate,
                  name: '客观题得分率'
                }
              ]
            },
            {
              ...
              data: [
                { value: resp.data.subjectScoreRate, name: '主观题得分率' }
              ]
            }
          ]
        }
